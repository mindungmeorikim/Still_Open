...
export const ResultSystem = {
  ...
  applyReward2xAdBonus(resultData = {}) {
    if (!resultData || Number(resultData.day) !== Number(GameState.day)) {
      console.warn("[ResultSystem] 광고 보상 적용 실패: 현재 정산 데이터가 아닙니다.", resultData);
      return {
        success: false,
        reason: "invalid_result",
        message: "현재 정산 광고 보상만 받을 수 있습니다."
      };
    }

    if (GameState.phase !== GAME_PHASE.RESULT) {
      console.warn("[ResultSystem] 광고 보상 적용 실패: 정산 단계가 아닙니다.", {
      ...
      return {
        ...
        message: "정산 화면에서만 광고 보상을 받을 수 있습니다."
      };
    }

    if (this.reward2xAppliedDay === GameState.day || resultData.reward2xAdApplied === true) {
      console.warn("[ResultSystem] 광고 보상 중복 적용 차단:", resultData);
      return {
        success: false,
        reason: "already_applied",
        message: "이미 오늘 광고 보상을 받았습니다."
      };
    }
    ...
    const diamondRewardResult = BMSystem.grantDiamond(diamondRewardAmount, {
      source: "result_ad_reward"
    });
    ...
    resultData.rewardAdDiamondAmount = diamondRewardResult.amount ?? diamondRewardAmount;
    resultData.diamond = diamondRewardResult.remainingDiamond;
    ...
    return {
      success: true,
      reason: "applied",
      message: extraBonus > 0
        ? `광고 보상 완료: 다이아 ${diamondRewardAmount.toLocaleString("ko-KR")}개 + 병맛 점수 ${extraBonus.toLocaleString("ko-KR")}`
        : `광고 보상 완료: 다이아 ${diamondRewardAmount.toLocaleString("ko-KR")}개를 받았습니다.`,
      baseBonus,
      extraBonus,
      diamondRewardAmount,
      diamond: diamondRewardResult.remainingDiamond,
  ...
  applyMentalRecoveryAdBonus(resultData = {}) {
    if (!resultData || Number(resultData.day) !== Number(GameState.day)) {
      console.warn("[ResultSystem] 멘탈 회복 광고 적용 실패: 현재 정산 데이터가 아닙니다.", resultData);
      return {
        success: false,
        reason: "invalid_result",
        message: "현재 정산 화면의 멘탈 회복만 받을 수 있습니다."
      };
    }

    if (GameState.phase !== GAME_PHASE.RESULT) {
      console.warn("[ResultSystem] 멘탈 회복 광고 적용 실패: 정산 단계가 아닙니다.", {
        phase: GameState.phase,
        resultData
      });
      return {
        success: false,
        reason: "invalid_phase",
        message: "정산 화면에서만 멘탈 회복 광고를 사용할 수 있습니다."
      };
    }

    if (
      this.mentalRecoveryAdAppliedDay === GameState.day ||
      resultData.mentalRecoveryAdApplied === true ||
      BMSystem.isMentalRecoveryAdUsedToday(GameState.day)
    ) {
      console.warn("[ResultSystem] 멘탈 회복 광고 중복 적용 차단:", resultData);
      return {
        success: false,
        reason: "already_applied",
        message: "이미 오늘 멘탈 회복 광고를 사용했습니다."
      };
    }

    const beforeMental = this.clamp(GameState.mental, 0, 100);

    if (beforeMental >= 100) {
      return {
        success: false,
        reason: "mental_full",
        message: "멘탈이 이미 최대치입니다."
      };
    }

    const recoveryAmount = Math.max(1, BMSystem.getMentalRecoveryAdAmount());
    const afterMental = this.clamp(beforeMental + recoveryAmount, 0, 100);
    const recoveredMental = afterMental - beforeMental;

    this.mentalRecoveryAdAppliedDay = GameState.day;
    BMSystem.recordMentalRecoveryAdUse(GameState.day);
    GameState.mental = afterMental;

    resultData.mentalRecoveryAdApplied = true;
    resultData.mentalRecoveryAmount = recoveredMental;
    resultData.mental = GameState.mental;
    resultData.mentalGap = GameState.mental - (Number(resultData.mentalGoal) || 1);
    resultData.mentalSuccess = GameState.mental > 0;
    ...
    return {
      success: true,
      reason: "applied",
      message: `광고 보상 완료: 멘탈 ${recoveredMental.toLocaleString("ko-KR")} 회복`,
      recoveryAmount: recoveredMental,
      mental: GameState.mental
    };
  },

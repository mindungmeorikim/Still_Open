...
const ALWAYS_WALKABLE_AREAS = Object.freeze([
  polygon("entry_sidewalk_corridor", [
    ...
    { x: 315, y: 713 },
    { x: 250, y: 676 }
  ], { requiredZoneId: "zone_basic", probeMode: "center" })
]);
...
export function isPointInCenterProbeWalkableArea(point = {}, areas = []) {
  return areas.some((area) => {
    return area?.probeMode === "center" && isPointInWalkableArea(point, area);
  });
}

export function arePointsInWalkableAreas(points = [], areas = []) {
  ...
  return safePoints.every((point) => isPointInWalkableAreas(point, areas));
}

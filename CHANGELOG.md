# 2026-07-12 — 손님 성향 상시 배지 제거

- 급한 손님·대량 구매·청결 중시·추천 상품 선호 성향 로직은 그대로 유지.
- 손님 머리 위 `급/대/청/추` 배지는 생성하지 않도록 변경.
- 이전 버전에서 이미 생성된 배지도 다음 렌더링 시 즉시 제거.
- CSS 안전 규칙으로 남아 있는 배지도 강제 숨김.
- 성향은 실제 행동이 발생하는 순간의 기존 말풍선과 토스트로만 안내.
- PWA 캐시 버전을 `still-open-release-20260712-v13-trait-badge-removed`로 갱신.
...
### Added
- GameAnalytics JavaScript SDK를 사용자가 분석 수집에 동의한 뒤에만 동적으로 불러오는 `AnalyticsSystem`을 추가했습니다.
- 최초 실행 시 플레이 데이터 수집 동의/거부 모달을 추가했습니다.
- 설정창에 플레이 데이터 수집 ON/OFF와 수집 항목 보기 기능을 추가했습니다.
- DEV Game Key와 Secret Key를 입력할 전용 설정 파일을 추가했습니다.
- SDK 초기화 완료 흐름에서 첫 연결 확인용 `game:start` Design Event를 전송하도록 준비했습니다.

### Safety
- 키가 비어 있거나 플레이스홀더인 동안 SDK를 초기화하거나 이벤트를 전송하지 않습니다.
- SDK 로딩 실패와 네트워크 오류가 게임 실행을 막지 않도록 방어 처리했습니다.
- 동의하지 않은 사용자는 SDK를 불러오지 않으며, 설정에서 언제든 전송을 중지할 수 있습니다.
- 플레이어/손님/알바 이동, 충돌, 좌표, 상호작용, 게임 경제 로직은 변경하지 않았습니다.

# CHANGELOG

## 2026-07-10 v8.10.24 - 구역 확장 조건 팝업 16:9 중앙 정렬/닫기 버튼 통일
- 구역 확장 조건 팝업을 16:9 게임 프레임 내부 중앙 기준으로 정렬했습니다.
- 팝업 최대 높이를 16:9 프레임 높이에 맞춰 제한해 BlueStacks/모바일 가로 화면에서 아래로 밀리지 않게 했습니다.
- 구역 확장 조건 팝업 닫기 버튼을 상점 닫기 버튼과 같은 이미지 버튼 규격으로 통일했습니다.
- 월드/카메라/game-world transform/오브젝트 좌표/충돌/상호작용/알바·손님 이동 좌표는 수정하지 않았습니다.


## [v8.10.23] 구역 확장 조건 순차 표시

### Changed
- `구역 확장 조건` 팝업이 2~4구역을 한 번에 전부 보여주지 않고, 현재 다음 확장 대상 1개만 보여주도록 변경했습니다.
- 1구역만 해금된 상태에서는 2구역 조건만, 2구역까지 해금되면 3구역 조건만, 3구역까지 해금되면 4구역 조건만 표시합니다.
- 잠긴 구역 자물쇠/구름을 눌러도 순차 확장 기준에 맞춰 다음 확장 대상 조건을 보여줍니다.
- 4구역까지 모두 해금되면 조건 카드 대신 `Coming Soon..` 안내를 표시합니다.

### Notes
- 월드/카메라/오브젝트 좌표/충돌/상호작용/알바·손님 이동 좌표는 변경하지 않았습니다.


## [v8.10.20] 좌측 HUD 구역 패널 간격 보정

### Fixed
- `구역` 패널이 `목표` 패널과 너무 붙어 보이지 않도록 살짝 아래로 내렸습니다.
- 구역 패널 폭/텍스트 크기는 직전 수정처럼 목표/매출 패널 기준을 유지했습니다.

### Notes
- 웹 브라우저와 BlueStacks/16:9에 공통 적용됩니다.
- 월드/카메라/오브젝트 좌표/충돌/상호작용은 변경하지 않았습니다.

## [v8.10.15] BlueStacks 16:9 구역 기반 카메라 프리셋 적용

### Changed
- BlueStacks/모바일 16:9에서 플레이어를 화면 중앙에 계속 두던 추적 방식을 기본 매장/확장 구역별 카메라 프리셋 방식으로 변경.
- 1구역만 열린 상태에서는 기본 매장 보기 좋은 구도를 우선해 냉장고/진열대가 바닥 중앙에 떠 보이는 문제를 완화.
- 2~4구역 해금 후에는 플레이어가 속한 해금 구역을 기준으로 카메라 타깃을 바꿔, 3구역 이동 시 3구역을 보도록 조정.
- 리사이즈 후에도 현재 플레이어 구역 기준 카메라가 다시 적용되도록 보정.
- 좌측 Day/목표/구역 HUD 기준선을 재정렬하고 영업 준비 칩은 Day 카드 내부 중앙에 고정.

### Unchanged
- 월드맵 에셋, 냉장고/진열대/계산대/청소도구/창고박스/발주박스 좌표는 변경하지 않음.
- 충돌 범위, 상호작용 범위, 손님/알바 이동 좌표는 변경하지 않음.

## [v8.10.13] Day Card Phase Chip Hotfix

### Fixed
- `영업 준비` 파란 칩만 Day 카드 내부 폭에 맞게 재고정했습니다.
- Day 카드 자체는 `overflow: visible`로 유지해 왼쪽 `목표`/`구역` 패널이 잘리지 않게 했습니다.

### Notes
- 월드/카메라, 오브젝트 좌표, 충돌 범위, 상호작용 범위는 변경하지 않았습니다.

## [v8.10.11] BlueStacks 16:9 Camera & HUD Polish

### Changed
- BlueStacks/모바일 16:9에서 기본 매장 전면에 있을 때 플레이어 머리 중심 추적 대신 매장 안쪽 기준 구도를 사용하도록 보정했습니다.
- 기본 매장만 열린 상태에서 냉장고/진열대가 화면 중앙 바닥에 떠 있는 것처럼 보이지 않도록 카메라 최소 줌과 기준점을 조정했습니다.

### Fixed
- Day 카드 하단의 `영업 준비` 칩이 카드 테두리 밖으로 삐져나오던 문제를 수정했습니다.
- 상단 일시정지 버튼만 다른 HUD 아이콘보다 크게 보이던 문제를 수정했습니다.

### Notes
- 월드맵 에셋, `game-world` transform, 오브젝트 좌표, 충돌 범위, 상호작용 범위, 손님/알바 이동 좌표는 변경하지 않았습니다.

## [v7.14.08] Target Revenue Balance Merge

### Changed
- Day 1~5 목표 매출을 완화했습니다: 25,000 / 40,000 / 55,000 / 72,000 / 90,000.
- 무한 모드 목표 매출 공식을 `90,000 + 추가 Day × 20,000` 기준으로 조정했습니다.

### Fixed
- 기존 저장 데이터에 남아 있는 이전 목표 매출값은 로드 시 새 밸런스 기준으로 자동 보정되도록 처리했습니다.

## [v7.14.07] Deploy Cleanup

### Fixed
- 배포본에서 F8 좌표 출력/충돌 박스 디버그 모드가 사용자에게 노출되지 않도록 비활성화했습니다.
- 클라이언트에 하드코딩되어 있던 테스트 쿠폰 코드 `LAUNCH-DIA-100`을 제거했습니다.
- 다이아 충전 UI의 “테스트 구매” 문구를 “결제 준비 중” 상태로 교체하고, 결제 SDK 미연결 상태에서는 구매 버튼을 비활성화했습니다.
- 게임 초기화 완료 콘솔 로그를 제거해 배포 콘솔 노출을 줄였습니다.


## [v7.14.06] Staff Entry Unstuck
- Fixed staff visually shaking at the entrance by moving the staff entry spawn to the walkable corridor.
- Added an entering-only escape guard so staff can walk out of an entrance spawn that is slightly outside walkable bounds instead of looping in place.
- Synced the UI default staff entry spawn with the StaffAssistSystem entry coordinate.
- Did not change player/customer movement, store object collision ranges, cleaning coordinates, or counter interaction ranges.


## [v7.14.05] Staff Entry Walk Spawn Fix
- Fixed: 알바 출근 시 입구 좌표가 렌더되자마자 청소 도구 옆 대기 좌표로 스냅되어 순간이동처럼 보이는 문제를 수정했습니다.
- Changed: 출근 시작 후 짧은 대기 프레임을 둔 뒤 `entering` 상태를 유지한 채 입구 → 매장 통로 → 대기 위치까지 걷도록 변경했습니다.
- Changed: 출근 이동이 끝난 뒤에만 `idle` 상태로 전환하고 자동 보조 체크를 예약하도록 분리했습니다.
- Note: 이동 가능 영역, 오브젝트 충돌 범위, 청소 좌표, 계산대 상호작용 범위는 수정하지 않았습니다.


## [v7.14.02] 계산대/청소 상호작용 우선순위 수정

### Fixed
- 1구역 계산대 근처에서 청소 구역과 상호작용 범위가 겹칠 때, 계산 가능한 손님이 있어도 청소가 먼저 잡히던 문제를 수정했습니다.
- 플레이어가 계산대 범위 안에 있으면 청소보다 계산대 상호작용을 우선 처리하도록 조정했습니다.
- 위생 상태가 이미 깨끗한 경우에는 청소 구역이 기본 상호작용 후보로 잡히지 않도록 수정했습니다.

### Changed
- 청소 구역 버튼은 실제 청소가 필요한 상태에서만 `data-player-action="cleaning"`을 유지하도록 조정했습니다.
- 기존 청소 좌표, 이동 가능 영역, 오브젝트 차단 범위는 변경하지 않았습니다.

[v7.14.01]
Changed
- 알바 이동 속도를 낮추고 프레임 지연 시 한 번에 크게 튀는 이동량을 제한해 순간이동처럼 보이는 이동을 완화.
- 알바가 이미 목표 위치 근처에 있을 때 불필요한 경유 루트를 타지 않고 바로 정착하도록 수정.
- 알바 대기 상태에서 걷기/청소도구/박스 흔들림 애니메이션이 남지 않도록 CSS 최종 override 추가.
- 이동 가능 영역, 오브젝트 차단 범위, 청소 좌표는 변경하지 않음.

[v7.14.00]
Changed
- 1구역 청소 구역의 X축은 유지하고 Y축만 30px 위로 이동 (x 735 유지, y 520 → 490).
- 알바 청소 정지 위치도 청소 구역 이동에 맞춰 Y축만 30px 위로 보정 (staffY 532 → 502).
- 계산대와 청소 상호작용이 겹치는 문제를 줄이기 위해 청소 상호작용 기준점을 위로 조정.
- 기존 오브젝트 차단 범위는 변경하지 않음.

[v7.13.99]
Changed
- 손님 입장 루트를 입구 → 매장 안쪽 경유지 → 해금 구역별 안전 웨이포인트 → 진열대 앞 좌표 순서로 재정리.
- 손님 진열대 목표 좌표가 이동 가능 영역 밖으로 잡히면 가장 가까운 walkable 좌표로 보정하도록 처리.
- 손님은 해금된 진열대 상품만 목표로 선택하는 기존 조건을 유지하고, 계산대 대기 좌표는 수정하지 않음.
- 플레이어/알바 이동 가능 영역과 기존 오브젝트 차단 범위는 수정하지 않음.
...
## [v7.14.03] 계산대/청소 상호작용 범위 분리
- Changed: 1구역 청소 상호작용 범위를 100px에서 55px로 축소해 계산대 앞에서 청소가 먼저 잡히는 현상을 줄였습니다.
- Changed: 계산대 상호작용 범위를 75px에서 90px로 보정해 계산대 앞에서는 계산 상호작용이 안정적으로 잡히도록 했습니다.
- Kept: 청소 도구 좌표, 이동 가능 영역, 기존 오브젝트 차단 범위는 변경하지 않았습니다.

## [v7.14.04] 알바 이동 속도/프레임 보정 완화
- Changed: 알바 자동 이동 속도를 낮춰 출근/보조 이동이 순간이동처럼 보이는 느낌을 줄였습니다.
- Changed: 프레임 지연 시 한 번에 이동하는 최대 보정값을 낮춰 알바가 쉭 튀어 보이는 현상을 완화했습니다.
- Changed: 목표점 도착 스냅 기준을 더 작게 줄여, 가까운 거리도 가능한 한 좌표 이동으로 보이도록 보정했습니다.
- Kept: 알바 이동 가능 영역, 오브젝트 차단 범위, 청소 좌표, 계산대/청소 상호작용 범위는 변경하지 않았습니다.

## [v7.14.07] 알바 이동 가능 영역 발 중심점 판정
- Changed: 알바 이동 가능 영역 판정을 발 주변 다중 포인트 검사에서 발 중심점 1개 검사로 단순화했습니다.
- Fixed: 입구/통로처럼 좁은 이동 가능 영역 경계에서 알바가 걸려 흔들리는 현상을 줄였습니다.
- Kept: 플레이어 이동 판정, 손님 루트, 청소 좌표, 계산대/청소 상호작용 범위, 기존 오브젝트 차단 범위는 변경하지 않았습니다.

## [v7.14.08] 알바 오브젝트 충돌 우선 보정
- Changed: 알바 이동 가능 영역 판정은 발 중심점 1개 기준을 유지하되, 계산대/진열대/창고/청소도구 같은 매장 오브젝트는 별도 하단 몸통 충돌 박스로 검사하도록 보정했습니다.
- Changed: 알바 전용 오브젝트 충돌 검사에서 계산대와 진열대에 소폭 안전 여백을 적용해, 알바가 계산대를 넘어다니는 것처럼 보이는 현상을 줄였습니다.
- Changed: 출근 루트에 하단 통로 경유점을 추가해 입구에서 대기 위치로 갈 때 계산대를 대각선으로 가로지르지 않도록 수정했습니다.
- Fixed: 출근 시작 보정 예외가 매장 오브젝트 충돌까지 무시하지 않도록 제한했습니다.
- Kept: 기존 공용 오브젝트 충돌 범위 데이터, 플레이어 이동 판정, 손님 루트, 청소 좌표, 계산대/청소 상호작용 범위는 변경하지 않았습니다.

## [v7.14.09] 상점 상시 접근 / 결과 팝업 확인 버튼 보정
- Fixed: 상점 구매/업그레이드 성공 팝업의 [확인] 버튼이 클릭되어도 닫히지 않을 수 있는 문제를 보정했습니다.
- Fixed: 성공 팝업 내부 요소 조회를 전역 `document` 기준이 아니라 현재 팝업 내부 기준으로 제한해 중복 ID/기존 DOM 잔류 상황에서도 안정적으로 동작하도록 수정했습니다.
- Changed: 상점 버튼은 영업 중/마감/정산 상태에서도 비활성화되지 않게 변경해 게임 진행 중 언제든 상점에 접속할 수 있도록 했습니다.
- Kept: 튜토리얼 중에는 기존 `guardTutorialAction` 규칙을 유지해 현재 튜토리얼 단계가 아닌 버튼 클릭은 계속 차단됩니다.

## [v8.10.3] 배포 전 상점 결과 팝업 닫기 보정

### Fixed
- 상점 구매/업그레이드 성공 팝업의 [확인] 버튼이 일부 상황에서 닫히지 않던 문제를 보정했습니다.
- 튜토리얼 입력 가드가 켜진 상태에서도 상점 구매 확인/결과 확인 버튼은 정상 클릭되도록 예외 처리했습니다.
- 상점 결과 팝업이 닫힐 때 상점 카드, 상품 카드, 재고/알바 요약 UI를 즉시 재렌더링하도록 보정했습니다.
- 성공 팝업의 hidden 상태가 CSS 우선순위에 밀리지 않도록 최종 override를 추가했습니다.

## [v8.10.4] BlueStacks 16:9 월드 시각 깊이 보정

### Changed
- BlueStacks/작은 16:9 가로 화면에서 기본 매장 카메라가 너무 멀어 보이지 않도록 반응형 추적 줌 기준을 보정했습니다.
- 플레이어/손님/알바/진열대/냉장고/계산대/창고박스/발주박스/청소도구의 화면 z-index를 월드 Y축 깊이 기준으로 동적 정렬하도록 보완했습니다.

### Fixed
- 작은 16:9 화면에서 플레이어가 오브젝트 위로 올라간 것처럼 겹쳐 보일 수 있는 시각 문제를 줄였습니다.

### Kept
- 오브젝트 좌표, 충돌 범위, 상호작용 범위, 손님/알바 이동 좌표, 월드맵 에셋 좌표는 변경하지 않았습니다.

[v8.10.3] BlueStacks 16:9 기본 매장 카메라 구도 보정
- Changed: BlueStacks/모바일 16:9 단칸 매장 화면에서 냉장고/진열대가 화면 중앙에 떠 보이지 않도록 기본 매장 카메라 최소 줌을 보정.
- Changed: 기본 매장 중심 카메라의 세로 기준점을 소폭 조정해 웹 브라우저에서 의도했던 1구역 구도에 가깝게 표시.
- Kept: 오브젝트 좌표, 충돌 범위, 상호작용 범위, 손님/알바 이동 좌표, 팝업/HUD 규칙은 변경하지 않음.

[v8.10.12] Fixed
- Day 카드의 `overflow:hidden` 때문에 좌측 목표/구역 패널이 같이 잘리던 문제를 복구.
- `영업 준비` 칩은 카드 내부에서만 말줄임/폭 제한되도록 유지.
- 월드/카메라/오브젝트 좌표/충돌/상호작용 범위는 수정하지 않음.

## [v8.10.14] BlueStacks 좌측 HUD 정렬/크기 보정
- Fixed: `영업 준비` 파란 칩이 Day 카드 검은 배경 밖으로 튀어나오지 않도록 칩만 카드 내부 중앙에 absolute 고정.
- Changed: 목표 패널과 구역 패널의 폭을 통일하고, 매출 금액/구역명 텍스트 크기를 같은 중요도로 맞춤.
- Safe: 월드/카메라/game-world transform/오브젝트 좌표/충돌 범위/상호작용 범위는 수정하지 않음.

## [v8.10.16] BlueStacks 1구역 오브젝트 배치 순차 보정
### Changed
- 웹 브라우저 기본 좌표는 유지하고, 작은 16:9 가로 화면에서만 1구역 냉장고/기본 매대에 반응형 보정값을 적용했습니다.
- 1구역 보정값은 렌더링 위치, 플레이어 상호작용 위치, 충돌 박스, 손님/알바 접근 좌표가 같은 이동량을 공유하도록 연결했습니다.
- Day 카드/목표/구역 좌측 HUD는 작은 16:9에서 같은 왼쪽 기준선과 폭을 쓰도록 다시 고정했습니다.

### Not Changed
- 웹 브라우저 기본 오브젝트 좌표와 월드/카메라/game-world transform은 변경하지 않았습니다.
- 2~4구역 오브젝트는 아직 보정하지 않았습니다. 1구역 검수 후 같은 방식으로 순차 적용합니다.

## [v8.10.17] BlueStacks 1구역 알바/손님 좌표 동기화 보강
### Fixed
- 1구역 BlueStacks 전용 냉장고/진열대 보정값을 알바 자동 보충 목표 좌표에도 확실히 적용하도록 보강했습니다.
- 알바가 PlayerActionSystem의 진열 슬롯을 읽을 때도 현재 화면 모드의 반응형 진열대 위치를 다시 병합해, 예전 좌표로 이동하지 않도록 수정했습니다.
- 손님 NPC의 쇼핑 목표 좌표와 렌더링 좌표가 현재 반응형 진열대 stand 좌표를 우선 사용하도록 보정했습니다.
- 리사이즈/BlueStacks 화면 전환 후에도 손님 렌더링이 저장된 예전 targetX/targetY보다 현재 진열대 배치 좌표를 우선 참조하도록 수정했습니다.

### Not Changed
- 웹 브라우저 기본 좌표, 2~4구역 오브젝트 좌표, 계산대/청소도구/창고박스/발주박스 좌표는 변경하지 않았습니다.
- 월드/카메라/game-world transform, 충돌 프리셋, 상점창/발주창 팝업은 변경하지 않았습니다.

## [v8.10.18] 좌측 HUD 공통 레이아웃 복구
### Fixed
- Day 카드/목표 패널/구역 패널 레이아웃을 웹 브라우저와 BlueStacks/16:9에서 같은 기준으로 적용하도록 복구했습니다.
- 작은 16:9 전용으로만 들어갔던 128px 축소값 때문에 어제 맞춰둔 좌측 HUD가 다시 달라지던 문제를 마지막 CSS override로 덮어썼습니다.
- 파란 `영업 준비` 칩은 Day 카드 내부 중앙에 고정하고, 목표 매출 금액과 구역명 텍스트 크기를 다시 통일했습니다.

### Not Changed
- 월드/카메라/game-world transform, 오브젝트 좌표, 충돌 범위, 상호작용 범위, 알바/손님 이동 좌표는 변경하지 않았습니다.

## v8.10.19 - 좌측 HUD 구역 패널 크기 통일
- 구역 패널 폭을 목표/매출 패널과 같은 136px로 맞췄습니다.
- `기본 매장` 텍스트는 매출 금액과 같은 중요도/크기 기준을 유지했습니다.
- 웹 브라우저와 BlueStacks/16:9에 공통 적용되며, 월드/카메라/오브젝트 좌표/충돌/상호작용은 수정하지 않았습니다.

## 2026-07-10 - 구역 확장 조건 UX 개선
- 왼쪽 구역 카드의 `확대 보기` 문구를 `확장 조건` 버튼으로 변경하고, 클릭 시 고정 팝업에서 2~4구역 확장 조건을 확인하도록 변경.
- 잠긴 구역의 자물쇠/구름/구역 라벨을 클릭해도 월드 줌아웃 없이 같은 `구역 확장 조건` 팝업이 열리도록 변경.
- 확장 가능한 구역이 생기면 왼쪽 구역 카드에 알림 배지와 `확장 가능` 안내가 표시되도록 추가.
- 월드/카메라/game-world transform, 오브젝트 좌표, 충돌/상호작용/손님/알바 좌표는 변경하지 않음.

## 2026-07-10 - v8.10.22 구역 확장 조건 클릭 핫픽스
- 좌측 구역 카드의 `확장 조건` 버튼이 기존 `pointer-events: none` 스타일에 막혀 눌리지 않던 문제를 수정했습니다.
...
## 2026-07-11 - 모달 포커스 및 aria-hidden 경고 수정

- 일시정지 버튼/계속하기 오버레이를 숨기기 전에 내부 포커스를 안전하게 해제하도록 수정
- 상단 메뉴 재구성 시 일시정지 버튼을 UIManager가 임의로 숨기지 않고 PauseSystem이 전담하도록 수정
- 창고 재고 모달과 상점 구매 결과 모달에 inert 및 포커스 복귀 처리 추가
- 숨겨진 모달 내부 버튼에 포커스가 남아 발생하던 Chrome `Blocked aria-hidden` 경고 제거
- 서비스워커 캐시 버전을 `v8-a11y-focus-fix`로 갱신


## 2026-07-12 — Day별 손님 흐름·진상 UX·알바 가치·첫 로그인 보상 통합

### 1차: 손님 흐름과 진상 UX
- Day 1~5 및 무한모드 손님 수, 가변 생성 간격, 연속 입장 확률, 동시 활성 수, 혼잡 재확인 시간을 Day별 프로필로 분리.
- Day 1은 18명, 4.5~10.5초 가변 간격, 20% 연속 입장, 최대 3명, 종료 40~55초 전 2명 러시로 설정.
- Day 2는 21명/최대 4명, Day 3은 24명/최대 5명으로 증가시켜 계산·보충 업무가 점차 밀리도록 조정.
- 계산 대기열, 긴급 재고, 위생, 모달 상태를 합산한 업무 압박도에 따라 다음 손님을 짧게 지연하도록 보정.
- Day 1의 6~9번째 손님 중 진상 1명을 보장하고, 그 외 Day 1 진상 추가 생성을 차단.
- 첫 계산 완료, 계산대 2초 대기, 튜토리얼/다른 모달 미노출 조건을 통과한 뒤 진상 이벤트가 열리도록 안전 조건 추가.
- 진상 선택창은 모든 Day에서 무제한 유지. 3초를 넘기면 만족도 -1을 한 번만 적용하고 자동 선택·자동 종료는 하지 않음.
- 진상 등장 자체의 멘탈 선차감을 제거하고 선택지 결과만 실제 멘탈·만족도에 반영.
- Day 1 부정 선택지 피해는 만족도/멘탈 항목당 최대 -2로 완화.
- 진상 모달과 일반 모달 종료 후 손님 인내심·영업시간에 조작 복귀 보호시간 적용.
- 계산 대기 말풍선에 인내심 단계와 게이지를 표시하고, 경고/위험 단계 시각 피드백 추가.
- Day 1 긍정 손님 1명을 보장하고 이후 Day에는 낮은 확률로 등장. 계산 완료 시 팁 300골드와 만족도 +1 제공.

### 2차: 업무 겹침과 알바 가치
- 손님 계산 누적에 따른 매장 이용량 기반 위생 압박 추가.
  - Day 1: 12번째 계산 시 위생 -10.
  - Day 2: 8·16번째 계산 시 각 위생 -30.
  - Day 3: 6·12·18번째 계산 시 각 위생 -20.
  - Day 4~5와 무한모드는 더 짧은 간격으로 단계 상승.
- 알바 보충 완료 시 상품명·수량, 청소 완료 시 청소 지점·위생 회복량을 즉시 표시.
- 알바의 창고 이동, 진열 보충, 청소 도움 횟수를 별도 집계하고 정산창에 한 줄 요약으로 표시.
- 수동 일시정지와 상점/설정/보상함 모달이 겹쳐도 모달이 닫히기 전 영업시간이나 손님 흐름이 재개되지 않도록 정지 소유권 경합 수정.

### 3차: 첫 로그인 보상과 운영 분석
- 로그인/플랫폼 SDK 사용자 ID를 우선 사용하고 미연결 시 설치별 익명 ID를 생성하는 UserIdentitySystem 추가.
- 첫 로그인 지원금 다이아 30개를 보상함에 계정/설치 범위 1회 지급.
- 새 게임 초기화 및 저장 불러오기 후에도 수령 이력을 분리 보존하여 재지급 차단.
- 수령 후 상점 사용처 안내 메시지 추가.
- GameAnalytics 사용자 ID를 초기화 전에 설정하고 Day 진행, 손님 이탈 이유, 평균 대기시간, 최대 대기열, 품절 시간, 진상 응답시간/3초 초과율, 선택지, 알바 고용/도움, 첫 보상 지급·수령·첫 다이아 사용처 이벤트 추가.
- PWA 캐시 버전을 `still-open-release-20260712-v10-customer-ux-reward`로 갱신하고 이번 변경 핵심 파일을 앱 셸에 추가.
...
### 주의
- 현재 서버 검증이 없으므로 앱 데이터 완전 삭제 또는 다른 기기에서는 설치 ID가 새로 생성될 수 있음. 다중 기기 계정당 1회를 강제하려면 로그인 서버/보상 서버의 수령 이력 연동이 필요함.

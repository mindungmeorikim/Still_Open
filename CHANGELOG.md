## [v8.10.10] 16:9 Popup Coverage Fix

### Fixed
- 발주창과 상점창은 기존 전용 레이아웃을 유지하고, 브리핑/정산/알바 고용/업그레이드/보상함/도움말/설정/상점 구매 확인/엔딩/무한모드 게임오버/손님 이벤트 모달만 16:9 게임 프레임 안에 들어오도록 보정했습니다.
- 브리핑창의 `발주하러 가기` 버튼이 모바일/BlueStacks 16:9 화면 하단에서 잘리지 않도록 sticky 하단 버튼과 내부 스크롤 처리를 강화했습니다.

### Not Changed
- 메인 월드, 카메라, `game-world` transform, 오브젝트 좌표, 충돌 범위, 상호작용 범위, 손님/알바 이동 좌표는 변경하지 않았습니다.

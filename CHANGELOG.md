...
## [v7.13.72] 알바 청소 이동 장애물 회피 보완

### Fixed
- 알바생이 청소 스팟으로 이동할 때도 계산대, 진열대, 손님 NPC를 이동 충돌 대상으로 검사하도록 보완했습니다.
- 기존 계산대 대기 손님만 회피하던 충돌 판정을 전체 손님 DOM 위치 기반 충돌 판정으로 확장했습니다.
- DOM 위치를 얻지 못하는 경우에도 계산대 대기열/진열대 주변 손님 위치를 fallback 충돌 영역으로 처리하도록 보완했습니다.

### Changed
- 청소 이동 기본 경로를 중앙/하단 안전 통로 경유 방식으로 정리했습니다.
- 1~4구역 청소 스팟 접근 시 구역별 접근 경유점을 사용해 계산대나 진열대를 가로지르는 이동을 줄였습니다.

### Maintained
- 알바 진열 보충 루트, 창고 박스 열림/닫힘, 재고 들기 표시, 플레이어 이동, 발주 박스 동선은 변경하지 않았습니다.

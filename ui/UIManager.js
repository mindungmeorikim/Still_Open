...
renderDebugCollisionBoxes() {
  ...
  normalizeSanitationState(state = null) {
    ...
    const sourceZoneId = sourcePoint?.zoneId ?? source.dirtyZoneId;
    const latestCleaningPoint = getCleaningPointByZoneId(sourceZoneId);
    const activeCleaningPoint = sourcePoint
      ? {
        ...latestCleaningPoint,
        ...sourcePoint,
        // 저장 데이터에 예전 청소 도구함 좌표가 남아 있어도 최신 배치 좌표를 우선 사용한다.
        x: Number(latestCleaningPoint.x),
        y: Number(latestCleaningPoint.y),
        staffX: Number(latestCleaningPoint.staffX ?? sourcePoint.staffX ?? latestCleaningPoint.x),
        staffY: Number(latestCleaningPoint.staffY ?? sourcePoint.staffY ?? latestCleaningPoint.y)
      }
      : latestCleaningPoint;
  ...
  renderCleaningZone() {
    ...
    cleaningZone.setAttribute("aria-label", "주변 청소");
